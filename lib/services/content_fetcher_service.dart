  ...
  Future<String> fetchYouTubeTranscript(String urlString) async {
    ...
    try {
      ...
      if (manifest.tracks.isEmpty) {
        throw Exception('No closed captions (manual or auto-generated) found for this video.');
      }

      // Helper to safely find a track or return null
      ClosedCaptionTrackInfo? findTrack(bool Function(ClosedCaptionTrackInfo) test) {
        try {
          return manifest.tracks.firstWhere(test);
        } catch (e) {
          return null;
        }
      }
      ...
      trackInfo = findTrack((t) => t.language.code == 'en' && !t.isAutoGenerated);

      // 2. If not found, try any other manual non-English caption
      trackInfo ??= findTrack((t) => t.language.code != 'en' && !t.isAutoGenerated);

      // 3. If still not found, try auto-generated English captions
      trackInfo ??= findTrack((t) => t.language.code == 'en' && t.isAutoGenerated);
      
      // 4. If still not found, try any other auto-generated non-English caption
      //    (or simply the first available track if the above fail, which will likely be auto-generated if no manual ones were found)
      trackInfo ??= findTrack((t) => t.isAutoGenerated); // Prioritize any auto-generated before falling to .first

      // 5. Fallback to the absolute first track if all specific searches fail (final safeguard)
      trackInfo ??= manifest.tracks.first;

      if (trackInfo == null) { 
          throw Exception('Could not select a suitable caption track despite tracks being available. This should not happen.');
      }

      debugPrint('Fetching captions in language: ${trackInfo.language.name} (Auto-generated: ${trackInfo.isAutoGenerated})');
